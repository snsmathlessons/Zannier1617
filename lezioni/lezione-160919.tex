...
\notamargine{La regolarità nei complessi è molto più forte che non nei reali. Essere olomorfe è davvero tanta roba in più che non essere
  $C^\infty$}
...
\begin{lemma}
  ...
  \notamargine{Sapendo infatti che $\exists \alpha, \beta$ tali che $f(z + \alpha) = f(z)$ e che $f(z + \beta) = f(z)$ $\forall z$ si ha
    $f((z + \alpha) + \beta) = f(z + \alpha) = f(z)$}
\end{lemma}
...
\begin{enumerate}
  \item $L_f$ è discreto
  \notamargine {Infatti, se $L_f$ non fosse discreto, arei molti punti in cui $f$ assume lo stesso valore che si accumulerebbero da qualche
    parte, quindi per il principio di continuazione analitica $f$ sarebbe costante}
...
\notamargine{Il norme curve ellittiche è dovuto al fatto che sono saltate fuori storicamente nel calcolo della lunghezza di archi di
  ellisse: data un'ellisse $y^2 = 1 - \alpha x^2$, per calcolare la lunghezza di un arco si giunge a:
...
\notamargine{Si può applicare il Teorema del Dini reale per costruire la funzione ``esplicita''. Quindi se ne può calcolare il differenziale
e notare che è della forma giusta per garantire l'olomorfia della funzione}
...
\notamargine{Ricordiamo infatti che se $R(t)$ è funzione razionale allora $\int R(t) \de t$ si può scrivere come funzione razionale più una
  combinazione di logaritmi traslati}
...
\notamargine{Ricordiamo che la Lemniscata è, fissati due punti, il luogo dei punti sul piano che hanno prodotto delle distanze fissato (dai
  due punti)}
...
\notamargine{Questo perché restringendo l'equazione della curva ellittica alla retta per $P$ e $Q$ si ottiene un polinomio di terzo grado,
  che si annulla nei punti in cui passa per la cubica, e che sappiamo avere già due radici nel campo che stiamo considerando. Ha quindi
  anche la terza, ed in questa intersecherà un altro punto $R$}
...
\paragraph{Esempi di gruppi algebrici} Facciamo ora alcuni esempi di gruppi algebrici (li pensiamo tutti su $\bbC$):
\begin{itemize}
